...
public class SignOpinionFillServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			inData.setStatus(request.getParameter("status"));
			inData.setCheckId(request.getParameter("checkId"));
			inData.setCheckContent(request.getParameter("checkContent"));

			if (StringUtil.isEmpty(inData.getRecordId()) || StringUtil.isEmpty(inData.getCheckId())||StringUtil.isEmpty(inData.getDiagnoseId())||StringUtil.isEmpty(inData.getStatus())) {
