...
(function ($)
{
    ...
    $.ligerDefaults.Form.editorBulider = function (jinput)
    {
        ...
        else if (jinput.is(":text") || jinput.is(":password"))
        {
            ...
            switch (ltype)
            {
                case "select":
                case "combobox":
                // edit by pt at 2013-05-04 将form的prefixID传入，以区分不同form的下拉框
                	inputOptions.prefixID = p.prefixID;
                    jinput.ligerComboBox(inputOptions);
                    break;
                case "spinner":
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _buliderLabelContainer: function (field)
        {
			...
			//如果表单有验证必填项，则在Label标记红色‘*’。 modify by WYS
			if(field.validate  ){
				if(field.validate.required){
					out.push('<font color=red>*</font>');
				}
			}
        ...
        _buliderControl: function (field)
        {
            ...
            {
            	 //edit by pt at 2013-03-21 此处form下拉框渲染时，会生成两个同名的hidden,第一个没有值，使的下拉框数据无法传到后台
            	//0213-05-04 采用其它方式解决，此处暂回复原状
            	 out.push('<input type="hidden" id="' + p.prefixID + name + '" name="' + name + '" />');
            }
