...
import servlet.outDataBean.WaitCancelSignOutDataBean;
		...
		if (responseW.isSuccess()) {
			try {
				for (int i = 0; i < responseWretData.size(); i++) {
					...
					if(data.get("type").toString().equals("2")){
						WaitCancelSignOutDataBean outData = new WaitCancelSignOutDataBean();
						...
						outData.setRecordId(data.get("iCaseHisRecordId").toString());
						outData.setCreateTime(data.get("RequestTime").toString());
						cancelData.add(outData);
					}else{
...
//						outData.setUserId(data.get("userId").toString());
					...
					bookMap.put("recordId", data.get("iCaseHisRecordId").toString());
					bookMap.put("uniqueId", data.get("uniqueId").toString());
					
					outData.getBookList().add(bookMap);
					}
				}
				contain.put("waitCancelSign", cancelData);
				contain.put("count", cancelData.size());
				retData.add(0,contain);
