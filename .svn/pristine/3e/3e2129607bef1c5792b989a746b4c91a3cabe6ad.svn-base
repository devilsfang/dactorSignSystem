...
(function($) {
	...
	$.ligerDefaults.Tree = {
		...
		// 是否点击展开/收缩 按钮时才有效
		//edit by pt at 2013-07-24 添加是否静态树的标识
		isStaticTree:false
	};
	...
	$.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
			...
			$.ajax({
						...
						success : function(data) {
							if (!data)
								return;
							/*
							 * edit by pt 2013-05-13 @lxl 修改获取返回数据的格式，以便接受框架后面数据
							 */
							if (data.ret_data) {
								/*
								 * 采用层级加载
		...
		_getTreeHTMLByData : function(data, outlineLevel, isLast, isExpand) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				var o = data[i];
				if (o.isexpand == true || o.isexpand == "true")
				...
				if (g.hasChildren(o)) {
					...
					if (p.parentIcon) {
						...
						/*
						 * edit by pt at 2013-07-30 判断是否静态树，解决动态树初始foder打开的问题
						 */
						/*
						if(p.isStaticTree){
						...
						*/
							treehtmlarr.push(g._getParentNodeClassName(false)
									+ " ");
						//}
				...
				treehtmlarr.push('<span>' + o[p.textFieldName]
						+ '</span></div>');
				/*
				 * edit by pt at 2013-07-24 添加是否静态树标识，必须是静态树才能执行渲染下级节点的操作
				 */
				if (g.hasChildren(o)&&p.isStaticTree) {
					var isLastNew = [];
					...
					if (p.autoCheckboxEven) {
						...
						if ($(obj).hasClass("l-checkbox-unchecked")) {
							...
							$(".l-children .l-checkbox", treeitem)
									//@lxl 2013-06-08去掉中间状态 源代码为: .removeClass("l-checkbox-incomplete l-checkbox-unchecked")
									.removeClass("l-checkbox-unchecked")
									.addClass("l-checkbox-checked");
							g.trigger('check', [{
												data : treenodedata,
						...
						// 状态：选中
							...
							$(obj).removeClass("l-checkbox-checked")
									.addClass("l-checkbox-unchecked");
							$(".l-children .l-checkbox", treeitem)
									//@lxl 2013-06-08去掉中间状态 源代码为: .removeClass("l-checkbox-incomplete l-checkbox-checked")
									.removeClass("l-checkbox-checked")
									.addClass("l-checkbox-unchecked");
							g.trigger('check', [{
												data : treenodedata,
		...
		_setParentCheckboxStatus : function(treeitem) {
			...
			if (isCheckedComplete) {
				treeitem
						...
						.prev()
						.find(".l-checkbox")
						//@lxl 2013-06-08去掉中间状态 源代码为: .removeClass("l-checkbox-unchecked l-checkbox-incomplete")
						.removeClass("l-checkbox-unchecked")
						.addClass("l-checkbox-checked");
				...
				treeitem
						...
						.prev()
						.find("> .l-checkbox")
						//@lxl 2013-06-08去掉中间状态 源代码为: .removeClass("l-checkbox-checked l-checkbox-incomplete")
						.removeClass("l-checkbox-checked")
						.addClass("l-checkbox-unchecked");
			} else {
				treeitem.parent().prev().find("> .l-checkbox")
						.removeClass("l-checkbox-unchecked l-checkbox-checked")
						//@lxl 2013-06-08去掉中间状态 源代码为: .addClass("l-checkbox-incomplete");
						.addClass("l-checkbox-checked");
			}
			if (treeitem.parent().parent("li").length > 0)
