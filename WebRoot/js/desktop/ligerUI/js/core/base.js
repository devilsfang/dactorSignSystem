    ...
    window.liger = $.ligerui = {
        ...
        add: function (manager)
        {
            ...
            if (this.managers[manager.id]){
            	...
            	//delete $.ligerui.managers[manager.id];
            	/*edit by pt at 2013-0315;
            	 * dialog关闭时,仅删除了及其下级节点的div，而在ligerUI的组件管理器中的组件信息未能删除，导致重新打开dialog时，会提示下级节点的ID在管理器中已存在*/
        ...
        run: function (plugin, args, ext)
        {
            ...
            //pt comments 过滤组件名称 如ligerForm,转化为Form
            ...
            if (this == null || this == window || ext.isStatic)
            {
                ...
                //pt comments 动态 new 组件，并传入ligerDefaults.xx(如 ligerDefaults.Form) 控件模型参数
            ...
            // pt comments 如果组件是XXManager 如ligerGetGridManager,则从组件管理器中返回组件对象
            if (/Manager$/.test(plugin)) return $.ligerui.get(this, ext.idAttrName);
            this.each(function ()
            {
            	//pt comments 判断组件是否已经存在，如果已存在,则返回(不再创建)
                ...
                //pt comments 控件模型参数（ligerDefaults.xx ligerDefaults.xxString）复制给 P
                var p = $.extend({}, $[ext.defaultsNamespace][plugin] || {}
                , $[ext.defaultsNamespace][plugin + 'String'] || {}, options || {});
                //pt comments 将html element 对象放入 p[element id]
                if (ext.propertyToElemnt) p[ext.propertyToElemnt] = this;
                //pt comments 为拥有element主体的插件new 对象 (比如drag、resizable等辅助性插件就不拥有)
                if (ext.hasElement)
                {
