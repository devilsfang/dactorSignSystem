...
(function($) {
	...
	$.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
		...
		_render : function() {
			...
			g.valueField = null;
			if (p.valueFieldID) {
				// edit by pt at 2013-0504
				// 修改form表单创建下拉框时，重复创建隐藏值的问题,将form的prefixID传入，以区分不同form的下拉框
				if (p.valueFieldID != "" && p.valueFieldID != null) {
					g.valueField = $("#" + p.prefixID + p.valueFieldID
							+ ":input");
				} else {
					g.valueField = $("#" + p.valueFieldID + ":input");
				}
				// g.valueField = $("[name='"+p.valueFieldID+"']" +":input");
			...
			if (g.select) {
				...
				$.ajax({
					...
					success : function(data) {
						// edit by pt at 2013-03-20 修改下拉框取数据格式
						g.data = [{
							id : "",
							text : "请选择"
						}];//
						for (var i = 0; i < data.ret_data.length; i++) {
							g.data.push(data.ret_data[i]);
						}
						g.setData(g.data);
		...
		setData : function(data) {
			...
			$(".l-table-checkbox input:checkbox", g.selectBox)
					.change(function() {
						...
						if (!p.isMultiSelect) {
							if (this.checked) {
								$("input:checked", g.selectBox).not(this)
										.each(function() {
											this.checked = false;
											$(".l-checkbox-checked",
													$(this).parent())
