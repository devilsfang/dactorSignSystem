...
(function ($)
{
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        active: function ()
        {
            ...
            if (g.minimize)
            {
            	//edit by pt at 2013-03-21 修改最小化后再次恢复窗体的大小
                var width = g.width==""?g._width:g.width, height = g.height==""?g._height:g.height, left = g._left, top = g._top;
                if (g.maximum)
                {
                    width = $(window).width();
        ...
        _removeDialog: function ()
        {
            ...
            if (p.showType && p.fixedType)
            {
                g.dialog.animate({ bottom: -1 * p.height }, function ()
                {
                    g.dialog.remove();
                    /*
	                 * edit by pt at 2013-03-21
	                 * 原代码此处仅销毁了dialog及其下级节点的div，而在ligerUI的组件管理器中的组件信息未能删除，导致重新打开dialog时，会提示下级节点的ID在管理器中已存在
	                 * 故增加从ligerUI的组件管理器中删除组件的代码
	                 */
	                delete l.managers[g.id];
                });
            } else
            {
                g.dialog.remove();
        ...
        close: function ()
        {
            var g = this, p = this.options;
            /*
             * add by pt at 2013-04-19
             * 添加窗体关闭事件 myBeforeClose
             */
            if(g.trigger('myBeforeClose',g) == false) return;
        ...
        _setUrl: function (url)
        {
            ...
            if (p.load)
            {
            	/*edit by pt at 2013-03-14解决ligerDialog重复load的问题
