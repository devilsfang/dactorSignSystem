							...
							pages : [
									{
										...
										content : [
													...
													columns : [ {
														cells : [ {
															render : function(gridData) {
																...
																var dataBase = gridData.waitCancelSign;
																$(dataBase)
																		.each(
																				function(
																						h,
																					...
																					var searchInfo = $("<div id='cancelInfo'></div>");
																					var searchImg = $("<div id='searchImg'></div>");
																					var searchName = $("<div id='bookName'></div>");
																					...
																					searchInfo
																							.bind(
																									eClick,
																									function() {
																										window.location.href = "/dssweb/pages/detail/record.html?model=3&userId="
																														+ myData.userId
																														+ "&"
																														+ SetUrlParam(dataBase)
																														+ "&"
																														+ SetUrlParam(data);
																												+ SetUrlParam(data);
																									});
												...
												},
												{
													...
													onBeforBuild : function(
															page) {
														if(page.gridData==null||page.gridData.length==0) {
															$("#tabName0").text("待处理 ");
															if(thisPage!=null) thisPage.tabPages[0].pages[0].grids[0].gridData=[];
														}else{
															$("#tabName0")
																	.text(
																			"待处理 "
																					+ (page.gridData.count > 0 ? page.gridData.count
																							: ""));
															thisPage.tabPages[0].pages[0].grids[0].gridData=page.gridData[0];
															page.gridData.splice(0,1);
														}
														thisPage.tabPages[0].pages[0].reLoadData(0,0,'grid');
													},
													emptyText : function() {
														var add = $("<span style='position: relative; z-index:9999;font-size:16px;color:#169fd6;display: block;width:100%'>"
																+ "没有要审签的数据 </span>");
													...
													columns : [
															{
																...
																cells : [ {
																	render : function(
																		...
																		// =
																		// "/dssweb/pages/detail/record.html?model=1&"
																		// +
																		// SetUrlParam(item);
																		// });
																		userInfo
																				.bind(
																						eClick,
																						function() {
																							window.location.href = "/dssweb/pages/detail/patientInfo.html?model=1&"
																									+ SetUrlParam(item.bookList[0])
																									+ "&"
																									+ SetUrlParam(item);
																						});
																		$(
																						...
																						function(
																							...
																							smallContainer
																									.bind(
																											eClick,
																											function() {
																												window.location.href = "/dssweb/pages/detail/record.html?userId="
																														+ myData.userId
																														+ "&"
																														+ SetUrlParam(item)
																														+ "&"
													...
													columns : [ {
														cells : [ {
															render : function(
																...
																var rightType = $("<div id='type' style='background-color:"
																		+ skin.backgroundColor
																		+ " '></div>");
																...
																right
																		.bind(
																				eClick,
																				function() {
																					window.location.href = "/dssweb/pages/detail/record.html?model=2&"
																							+ SetUrlParam(item);
																				});
																middle
																		.bind(
																...
																sex
																		.append($("<span class='sex'>"
																				...
																				+ "</span>"));
																number
																		.append(item.patientCode);

																rightType
																		.append("<div id='heart'><img src='"
																				+ skin.icon
																				+ "'></div>"
																				+ "<div id='contentType'>"
																				+ skin.name
																...
																if (item.status == "通过") {
																	...
																	through
																			.addClass("notThrough");
																	through1
																			.addClass("notThrough1");
																} else {
																	through1
																			.append("处理中…");
																	through
																			.addClass("unknowThrough");
																	through1
																			.addClass("unknowThrough1");
																}
																clock
										...
										content : [
												{
													...
													type : "checkBox",
													onChange:function(e){
														thisPage.tabPages[0].pages[2].grids[0].gridParam.hospitalStatus = e;
														thisPage.tabPages[0].pages[2].reLoadData(2, 0, "grid");
													},
													data : [ {
														id : "1",
														text : "住院患者"
...
function queryRecordByDate(date) {
	thisPage.tabPages[0].pages[1].grids[0].gridParam.checkDate = date.replace(
...
function getDateDiff1(dateTimeStamp) {
	var minute = 1000 * 60, hour = minute * 60, day = hour * 24, halfamonth = day * 15, month = day * 30, year = month * 12;
	var now = new Date().getTime();
	var sendDate = new Date(dateTimeStamp).getTime();
	var diffValue = now - sendDate;
	if (diffValue < 0) {
	}
	var yearC = diffValue / year, monthC = diffValue / month, weekC = diffValue
			/ (7 * day), dayC = diffValue / day, hourC = diffValue / hour, minC = diffValue
			/ minute;
	/*
	 * if (yearC >= 1) { result = parseInt(yearC) + "年前"; } else
	 */
	if (monthC > 1) {
		result = "30天以上";
	} else if (weekC > 1) {
